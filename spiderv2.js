...
function getServerRatio_v2(ns, server, neededRam, takePercent) {
	...
	let cycleThreads = Math.ceil((totalTime*1000)/100*totalThreads);

	let ratio = Math.floor(targetHackMoney/neededGB/totalTime*100)/100;
	if (isNaN(ratio)){
		ratio = null;
...
async function adjustTake(ns, inventory, totalReservedThreads, allMaxed, indexOfAlpha) {
	...
	/** adjusting Take */
	while(targetAlpha.adjustedRatio !== null && targetAlpha.adjustedRatio > targetBeta.adjustedRatio && totalReservedThreads < inventory.maxThreads) {
...
async function updateServerInventory(ns, serverList, inventory, subset) {
	...
	for (let server of serverList){
		...
		if (subset == 'targets') {
			...
			if (server == 'home') {
				ram -= Math.max(32, ram*.001); //sets home ram to have 32 gbs of reserve space or 0.1% whichever is larger
...
export async function main(ns) {
	...
	//Terminal Output
	ns.tprint('Have root access on ' + rootedServers.length +' servers.');
	...
	//Terminal Output
	ns.tprint("An additional " + serverList.length + ' servers on on the network');

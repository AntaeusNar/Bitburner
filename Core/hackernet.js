/** @param {NS} ns **/export async function main(ns) {
  ...
  while (true) {
      ...
      // wait until you have the cost + 20% over current money
      let min_money = getMoney();
      let wait = true;
      while (min_money + cost*1.2 > getMoney()) {
        if (wait) {
          ns.print("Waiting for " + (min_money+cost*1.2) + ' before Purchasing')
          wait = false;
        }
        await ns.sleep(1);
      }
